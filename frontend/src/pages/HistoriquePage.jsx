    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <select value={h || ''} onChange={e => onChange(`${e.target.value}:${m || '00'}`)}
                style={{
                    width: 44, padding: '5px 2px', borderRadius: 6, textAlign: 'center',
    ...
    return (
        <div style={{ position: 'relative', width }}>
            <select value={value} onChange={e => onChange(e.target.value)}
                style={{
                    width: '100%', padding: '7px 28px 7px 10px', borderRadius: 7,
...
function EditModal({ saisie, token, farms, onSaved, onClose, C, dark }) {
    ...
    const TH = ({ children, w, color }) => (
        <th style={{
            padding: '7px 5px', textAlign: 'center', fontSize: 10, fontWeight: 700,
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'inherit' }}>
                                ...
                                <tbody>
                                    {tours.length === 0 ? (
                                        ...
                                        }}>Aucun tour — cliquer sur Nouveau tour</td></tr>
                                    ) : tours.map((t, i) => (
                ...
                <td colSpan={20} style={{ padding: 0 }}>
                    <div style={{ margin: '0 0 0 32px', borderLeft: `3px solid ${C.green}30` }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'inherit' }}>
                            ...
                            <tbody>
                                {tours.length === 0 ? (
                                    <tr><td colSpan={14} style={{
                                        padding: '16px', textAlign: 'center',
                                        color: C.textDim, fontSize: 12
                                    }}>Aucun tour enregistré</td></tr>
                                ) : tours.map((t, i) => (
                                    <tr key={t.id}
                                        style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <select value={perPage} onChange={e => setPerPage(Number(e.target.value))}
                        style={{
                            padding: '7px 10px', borderRadius: 7, border: `1.5px solid ${C.border}`,
                                        ...
                                        <tr key={s.id}
                                            ...
                                            <td style={{ padding: '10px 8px', textAlign: 'center' }}>
                                                <button onClick={() => toggleExpand(s.id)}
                                                    style={{
                                                        background: expanded ? `${C.green}12` : C.toggleBg,
